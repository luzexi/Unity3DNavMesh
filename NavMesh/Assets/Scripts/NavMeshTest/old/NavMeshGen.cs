    ...
    public sealed class NavMeshGen
    {
        ...
        public NavResCode SaveNavMeshToFile(string filePath, List<Triangle> navTriangles)
        {
            ...
            foreach (Triangle navTri in navTriangles)
            {
                ...
                {
                    //保存网格ID
                    binWriter.Write(navTri.ID);
                    //保存网格的三角形顶点
                    for (int i = 0; i < 3; i++)
                    {
                        binWriter.Write(navTri.Points[i].x);
                        binWriter.Write(navTri.Points[i].y);
                    }

                    // 保存所有邻居边
                    for (int i = 0; i < 3; i++)
                    {
                        binWriter.Write(navTri.Neighbors[i]);
                    }

                    // 保存每条边中点距离
                    for (int i = 0; i < 3; i++)
                    {
                        binWriter.Write(navTri.WallDistance[i]);
                    }

                    // 保存中心点位置
                    binWriter.Write(navTri.CenterPos.x);
                    binWriter.Write(navTri.CenterPos.y);

                    //保存区域id
                    binWriter.Write(navTri.Group);
                }
            }
        ...
        public NavResCode LoadNavMeshFromFile(ref List<Triangle> navTriangles, BinaryReader binReader)
        {
            try
            {
                ...
                for (int i = 0; i < navCount; i++)
                {
                    ...
                    // 读取id
                    currTri.ID = binReader.ReadInt32();
                    // 读取多边形的顶点
                    for (int pNum = 0; pNum < 3; pNum++)
                    {
                        currTri.Points[pNum].x = binReader.ReadSingle();
                        currTri.Points[pNum].y = binReader.ReadSingle();
                    }
                    // 计算包围盒
                    currTri.CalcCollider();
                    // 读取邻居节点
                    for (int neighborId = 0; neighborId < 3; neighborId++)
                    {
                        currTri.Neighbors[neighborId] = binReader.ReadInt32();
                    }
                    // 读取每条边中点距离
                    for (int wall = 0; wall < 3; wall++)
                    {
                        currTri.WallDistance[wall] = binReader.ReadDouble();
                    }

                    // 读取中心点
                    Vector2 tempCenter = currTri.CenterPos;
                    tempCenter.x = binReader.ReadSingle();
                    tempCenter.y = binReader.ReadSingle();
                    currTri.CenterPos = tempCenter;

                    // 读取区域id
                    currTri.Group = binReader.ReadInt32();
        ...
        public NavResCode CreateNavMesh(List<Polygon> polyAll,ref List<Triangle> triAll)
        {
            ...
            Debug.Log("lstPoly " + polyAll.Count);

            Debug.Log("in1 " + allStartEdges.Count );
            ...
            for (int edgeIndex = 0; edgeIndex < allStartEdges.Count; edgeIndex++)
            {
                ...
                do
                {
                    ...
                    Line2D lAD = new Line2D(edge.PointStart, dtPoint);
                    Line2D lDB = new Line2D(dtPoint, edge.PointEnd);

                    //创建三角形
                    Triangle delaunayTri = new Triangle(edge.PointStart, edge.PointEnd, dtPoint);
                    //设置编号
                    delaunayTri.ID = index++;
                    // set group for island
                    delaunayTri.Group = edgeIndex;
                    // 保存邻居节点
//                     if (lastNeighborId != -1)
//                     {
...
//                     }
            ...
            for (int i = 0; i < triAll.Count; i++)
            {
                ...
                for (int j = 0; j < triAll.Count; j++)
                {
                    ...
                    if (result != -1)
                    {
                        tri.Neighbors[result] = triNext.ID;
                    }
                }
            }
...
//         }
        ...
        private bool IsPointVisibleOfLine(Line2D line, Vector2 point)
        {
            ...
            Vector2 sPnt = line.PointStart;
            Vector2 ePnt = line.PointEnd;
        ...
        private bool IsVisibleIn2Point(Vector2 sPnt, Vector2 ePnt)
        {
            ...
            foreach (Line2D edge in allEdges)
            {
                ...
                {
                    if ( !SGMath.IsEqualZero(sPnt - interPos)
                        && !SGMath.IsEqualZero(ePnt - interPos) )
                        return false;
                }
            }
            return true;
        ...
        private bool FindDT(Line2D line,out Vector2 dtPoint)
        {
            ...
            Vector2 ptA = line.PointStart;
            Vector2 ptB = line.PointEnd;
            ...
            Debug.Log("visible position " + visiblePnts.Count);
            ...
            do 
            {
                ...
                Circle circle = SGMath.CreateCircle(ptA, ptB, dtPoint);
                Rect boundBox = SGMath.GetCircleBoundBox(circle);

                //Step2. 依次访问网格包围盒内的每个网格单元：
                //若某个网格单元中存在可见点 p, 并且 ∠p1pp2 > ∠p1p3p2，则令 p3=p，转Step1；
                //否则，转Step3.
                float angOld =  (float)Math.Abs(SGMath.LineRadian(ptA, dtPoint, ptB));
                foreach (Vector2 pnt in visiblePnts)
                {
                    ...
                    float angNew = (float)Math.Abs(SGMath.LineRadian(ptA, pnt, ptB));
                    if (angNew > angOld)
                    {
                        dtPoint = pnt;
                        bContinue = true;
        ...
        private NavResCode InitData(List<Polygon> polyAll)
        {
            ...
            PolyResCode resCode = Polygon.UnionAll(ref polyAll);
            ...
            {
                if (poly.allPoints.Count < 3)
                    continue;

                AddPoint(poly.allPoints);
                AddEdge(poly.allPoints);

                // 更改算法，初始边只用孤岛的内边，这就强制要求使用的时候必须把可以行走的区域全部包围起来
                //if (poly.tag != 0)
                {
                    allStartEdges.Add(new Line2D(poly.allPoints[1], poly.allPoints[0]));
                }
            }
